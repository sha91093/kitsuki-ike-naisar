...
スネコスリ溜池（ID 27）は、生成された6つの連結変化領域を国土地理院の航空画像に重ねた
`docs/qa-27.html` でも確認できます。QA画面は候補確認専用で、元の池GeoJSONは変更しません。

昇交・降交の近接日ペアを4近傍で照合し、0・20・50 mバッファの開放水面積を比較する試作は
`scripts/estimate_open_water.py` で生成し、`docs/water-27.html` に表示します。植生下冠水と低一致ペアは確定面積へ含めません。

Sentinel-2 L2Aの緑・近赤外・短波赤外とSCLを池周辺だけ遠隔部分読み込みし、NDWIとMNDWIによる光学水域で照合する処理は
`scripts/validate_sentinel2.py` で生成し、`docs/sentinel2-27.html` に表示します。局所有効画素率95%未満の撮影は雲・雲影ありとして比較から除外します。
検索JSONがない環境ではEarth Search STACを自動検索し、画像全体を保存せず池周辺だけを公開COGから読み込みます。

光学水域を教師として昇交・降交別のHH/HV固定閾値を0.25 dB刻みで探索する処理は
`scripts/tune_nisar_thresholds.py` で実行し、`docs/threshold-27.html` に結果を表示します。6月29日と8月23日を調整に使い、8月13日と9月22日は未使用検証日として保持します。
同じ処理で多雨期からのΔHH・ΔHVも3×3画素中央値後に探索します。調整日だけでなく未使用日のIoUを確認し、時間差条件の過学習を判定します。
