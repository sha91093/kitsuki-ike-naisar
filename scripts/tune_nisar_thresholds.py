...
def tune(args: argparse.Namespace) -> tuple[pd.DataFrame, dict]:
    ...
    for sample in samples:
        ...
        for method, prediction in (("tuned_fixed", tuned), ("per_scene_otsu", otsu)):
        ...
        tp = tuned & sample["truth"]
        fp = tuned & ~sample["truth"] & analysis_mask
        fn = ~tuned & sample["truth"] & analysis_mask
        map_features.append(
            {
                "optical_date": sample["optical_date"],
                "nisar_date": sample["nisar_date"],
